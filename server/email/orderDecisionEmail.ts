...
export interface OrderDecisionEmailPayload extends OrderCreatedEmailPayload {
...
/** Email to the agent when their order is approved or rejected. */
export function buildOrderDecisionEmailHtml(p: OrderDecisionEmailPayload): string {
  ...
  const intro = approved
    ? `Good news — your order for <strong>${esc(p.customerName ?? 'the customer')}</strong> has been approved by ${esc(decidedLabel)}. You can proceed with fulfillment steps in Lamtex.`
    : `Your order for <strong>${esc(p.customerName ?? 'the customer')}</strong> was rejected by ${esc(decidedLabel)}. Review the details below and update the order if needed before resubmitting.`;
  ...
  const rejectionBlock =
    !approved && p.rejectionReason?.trim()
  ...
  const notesBlock = p.orderNotes?.trim()
    ? infoCard('Order notes', detailRow('Notes', p.orderNotes.trim()), `${p.orderNumber} · ${decisionKind} · notes`)
    : '';
  ...
  const ctaLabel = approved ? 'View approved order' : 'View order &amp; revise';
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.08);border:1px solid #e5e7eb;">
        ...
        <tr>
          <td style="padding:28px 32px;">
            ...
            ${rejectionBlock}
            ${infoCard('Order summary', orderSummaryRows, `${p.orderNumber} · ${decisionKind} · summary`)}
            ${logisticsRows ? infoCard('Delivery & payment', logisticsRows, `${p.orderNumber} · ${decisionKind} · delivery`) : ''}
            ${notesBlock}
            ...
            <div style="margin-top:28px;text-align:center;">
              <a href="${orderUrl}" style="display:inline-block;background:${ctaBg};color:#ffffff;text-decoration:none;padding:13px 32px;border-radius:8px;font-weight:600;font-size:15px;box-shadow:0 1px 2px rgba(0,0,0,.06);">${ctaLabel}</a>
            </div>
          </td>
        </tr>
        <tr>
