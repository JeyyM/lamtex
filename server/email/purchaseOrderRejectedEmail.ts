...
export interface PurchaseOrderRejectedEmailPayload extends PurchaseOrderSubmittedEmailPayload {
...
/** Email to the warehouse user who submitted a PO when it is rejected. */
export function buildPurchaseOrderRejectedEmailHtml(p: PurchaseOrderRejectedEmailPayload): string {
  ...
  const rejectionBlock = p.rejectionReason?.trim()
  ...
  const notesBlock = p.notes?.trim()
    ? infoCard('Notes', detailRow('Notes', p.notes.trim()), `${p.poNumber} · rejected · notes`)
    : '';
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.08);border:1px solid #e5e7eb;">
        <tr>
          <td style="background:linear-gradient(135deg,#dc2626,#991b1b);padding:28px 32px;">
            ...
            <p style="margin:8px 0 0;color:#fecaca;font-size:14px;">${esc(p.poNumber)} · ${esc(p.branchName ?? 'Branch')}</p>
          </td>
        </tr>
        <tr>
          <td style="padding:28px 32px;">
            ...
            ${rejectionBlock}
            ${infoCard('Purchase order summary', summaryRows, `${p.poNumber} · rejected · summary`)}
            ${notesBlock}
