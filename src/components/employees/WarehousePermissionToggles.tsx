...
import { toggleWarehousePermission } from '@/src/lib/permissions/employeeWarehousePermissions';

const PERMISSION_ICONS: Record<WarehousePermissionKey, LucideIcon> = {
  pageAccess: Eye,
};

export function WarehousePermissionToggleGrid({
  value,
  onChange,
  disabled,
}: {
  value: WarehousePermissionSet;
  onChange: (next: WarehousePermissionSet) => void;
  disabled?: boolean;
}) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
      {WAREHOUSE_PERMISSIONS.map((def) => {
        ...
        return (
          <button
            ...
            aria-pressed={on}
            onClick={() => onChange(toggleWarehousePermission(value, def.key))}
            className={cn(
...
export function WarehousePermissionSection({
  title = 'Warehouse',
  description = 'Control access to the Warehouse hub page.',
  value,
  savedValue,
  onChange,
  onSave,
  ...
  description?: string;
  value: WarehousePermissionSet;
  savedValue: WarehousePermissionSet;
  onChange: (next: WarehousePermissionSet) => void;
    ...
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="p-4 sm:p-6">
        {loading ? (
          ...
          <>
            <WarehousePermissionToggleGrid value={value} onChange={onChange} disabled={saving} />
