...
import { toggleReportsPermission } from '@/src/lib/permissions/employeeReportsPermissions';

const PERMISSION_ICONS: Record<ReportsPermissionKey, LucideIcon> = {
  pageAccess: Eye,
};

export function ReportsPermissionToggleGrid({
  value,
  onChange,
  disabled,
}: {
  value: ReportsPermissionSet;
  onChange: (next: ReportsPermissionSet) => void;
  disabled?: boolean;
}) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
      {REPORTS_PERMISSIONS.map((def) => {
        ...
        return (
          <button
            ...
            aria-pressed={on}
            onClick={() => onChange(toggleReportsPermission(value, def.key))}
            className={cn(
...
export function ReportsPermissionSection({
  title = 'Reports',
  description = 'Control access to the Reports page and all report tabs (overview, sales, products, inventory, agents).',
  value,
  savedValue,
  onChange,
  onSave,
  ...
  description?: string;
  value: ReportsPermissionSet;
  savedValue: ReportsPermissionSet;
  onChange: (next: ReportsPermissionSet) => void;
    ...
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="p-4 sm:p-6">
        {loading ? (
          ...
          <>
            <ReportsPermissionToggleGrid value={value} onChange={onChange} disabled={saving} />
