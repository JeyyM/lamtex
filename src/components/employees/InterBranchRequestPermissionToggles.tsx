...
import { toggleInterBranchRequestPermission } from '@/src/lib/permissions/employeeInterBranchRequestPermissions';

const PERMISSION_ICONS: Record<InterBranchRequestPermissionKey, LucideIcon> = {
  pageAccess: Eye,
  creation: PlusCircle,
  approvals: ThumbsUp,
  scheduling: Calendar,
...
export function InterBranchRequestPermissionToggleGrid({
  value,
  onChange,
  disabled,
}: {
  value: InterBranchRequestPermissionSet;
  onChange: (next: InterBranchRequestPermissionSet) => void;
  disabled?: boolean;
}) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
      {INTER_BRANCH_REQUEST_PERMISSIONS.map((def) => {
        ...
        return (
          <button
            ...
            aria-pressed={on}
            onClick={() => onChange(toggleInterBranchRequestPermission(value, def.key))}
            className={cn(
...
export function InterBranchRequestPermissionSection({
  title = 'Inter-branch Requests',
  description = 'Control what this employee can see and do on Inter-branch Request list and detail pages.',
  value,
  savedValue,
  onChange,
  onSave,
  ...
  description?: string;
  value: InterBranchRequestPermissionSet;
  savedValue: InterBranchRequestPermissionSet;
  onChange: (next: InterBranchRequestPermissionSet) => void;
    ...
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="p-4 sm:p-6">
        {loading ? (
          ...
          <>
            <InterBranchRequestPermissionToggleGrid value={value} onChange={onChange} disabled={saving} />
