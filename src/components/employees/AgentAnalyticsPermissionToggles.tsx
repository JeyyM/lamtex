...
import { toggleAgentAnalyticsPermission } from '@/src/lib/permissions/employeeAgentAnalyticsPermissions';

const PERMISSION_ICONS: Record<AgentAnalyticsPermissionKey, LucideIcon> = {
  pageAccess: Eye,
};

export function AgentAnalyticsPermissionToggleGrid({
  value,
  onChange,
  disabled,
}: {
  value: AgentAnalyticsPermissionSet;
  onChange: (next: AgentAnalyticsPermissionSet) => void;
  disabled?: boolean;
}) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
      {AGENT_ANALYTICS_PERMISSIONS.map((def) => {
        ...
        return (
          <button
            ...
            aria-pressed={on}
            onClick={() => onChange(toggleAgentAnalyticsPermission(value, def.key))}
            className={cn(
...
export function AgentAnalyticsPermissionSection({
  title = 'Agent Analytics',
  description = 'Control access to the Agent Analytics page (leaderboards, quotas, trends).',
  value,
  savedValue,
  onChange,
  onSave,
  ...
  description?: string;
  value: AgentAnalyticsPermissionSet;
  savedValue: AgentAnalyticsPermissionSet;
  onChange: (next: AgentAnalyticsPermissionSet) => void;
    ...
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="p-4 sm:p-6">
        {loading ? (
          ...
          <>
            <AgentAnalyticsPermissionToggleGrid value={value} onChange={onChange} disabled={saving} />
