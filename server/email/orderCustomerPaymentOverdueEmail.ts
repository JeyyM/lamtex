...
export interface OrderCustomerPaymentOverdueEmailPayload extends OrderCustomerApprovedEmailPayload {
  dueDate: string | null;
  daysOverdue: number;
  amountPaid: number;
  balanceDue: number;
...
/** Customer-facing email when their order payment is past due. */
export function buildOrderCustomerPaymentOverdueEmailHtml(
  p: OrderCustomerPaymentOverdueEmailPayload,
): string {
  const greetingName =
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.08);border:1px solid #e5e7eb;">
        <tr>
          <td style="background:linear-gradient(135deg,#b91c1c,#991b1b);padding:28px 32px;">
            ...
            <p style="margin:8px 0 0;color:#fecaca;font-size:14px;">Order ${esc(p.orderNumber)} · past payment terms</p>
          </td>
        </tr>
        <tr>
          <td style="padding:28px 32px;">
            ...
            ${infoCard('Order summary', orderSummaryRows, `${p.orderNumber} · customer · overdue`)}
            ${portalBlock}
