...
import { toggleSupplierPermission } from '@/src/lib/permissions/employeeSupplierPermissions';

const PERMISSION_ICONS: Record<SupplierPermissionKey, LucideIcon> = {
  pageAccess: Eye,
};

export function SupplierPermissionToggleGrid({
  value,
  onChange,
  disabled,
}: {
  value: SupplierPermissionSet;
  onChange: (next: SupplierPermissionSet) => void;
  disabled?: boolean;
}) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
      {SUPPLIER_PERMISSIONS.map((def) => {
        ...
        return (
          <button
            ...
            aria-pressed={on}
            onClick={() => onChange(toggleSupplierPermission(value, def.key))}
            className={cn(
...
export function SupplierPermissionSection({
  title = 'Suppliers',
  description = 'Control access to the Suppliers list and supplier detail pages.',
  value,
  savedValue,
  onChange,
  onSave,
  ...
  description?: string;
  value: SupplierPermissionSet;
  savedValue: SupplierPermissionSet;
  onChange: (next: SupplierPermissionSet) => void;
    ...
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="p-4 sm:p-6">
        {loading ? (
          ...
          <>
            <SupplierPermissionToggleGrid value={value} onChange={onChange} disabled={saving} />
