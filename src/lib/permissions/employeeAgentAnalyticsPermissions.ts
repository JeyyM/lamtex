...
export async function fetchEmployeeAgentAnalyticsPermissions(
  ...
  if (!id) return { ...ALL_AGENT_ANALYTICS_PERMISSIONS_GRANTED };

  const { data, error } = await supabase
    .from('employee_agent_analytics_permissions')
    .select('permissions')
    .eq('employee_id', id)
    .maybeSingle();

  if (error) {
    if (import.meta.env.DEV) console.warn('[employeeAgentAnalyticsPermissions] fetch', error.message);
    return { ...ALL_AGENT_ANALYTICS_PERMISSIONS_GRANTED };
  }
  if (!data?.permissions) return { ...ALL_AGENT_ANALYTICS_PERMISSIONS_GRANTED };
  return normalizeAgentAnalyticsPermissionSet(data.permissions);
}
