...
export async function fetchEmployeeSupplierPermissions(employeeId: string): Promise<SupplierPermissionSet> {
  ...
  if (!id) return { ...ALL_SUPPLIER_PERMISSIONS_GRANTED };

  const { data, error } = await supabase
    .from('employee_supplier_permissions')
    .select('permissions')
    .eq('employee_id', id)
    .maybeSingle();

  if (error) {
    if (import.meta.env.DEV) console.warn('[employeeSupplierPermissions] fetch', error.message);
    return { ...ALL_SUPPLIER_PERMISSIONS_GRANTED };
  }
  if (!data?.permissions) return { ...ALL_SUPPLIER_PERMISSIONS_GRANTED };
  return normalizeSupplierPermissionSet(data.permissions);
}
