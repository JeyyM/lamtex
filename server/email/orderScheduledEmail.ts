...
export interface OrderScheduledEmailPayload extends OrderCreatedEmailPayload {
...
/** Order scheduled — routed to executives, warehouse staff, or the assigned agent. */
export function buildOrderScheduledEmailHtml(p: OrderScheduledEmailPayload): string {
  ...
  const intro =
    target === 'executive'
      ? `Order <strong>${esc(p.orderNumber)}</strong> for <strong>${esc(p.customerName ?? 'the customer')}</strong> was scheduled by ${esc(scheduledLabel)}. Trip and schedule details are below.`
      : target === 'warehouse'
        ? `Order <strong>${esc(p.orderNumber)}</strong> for <strong>${esc(p.customerName ?? 'the customer')}</strong> is scheduled for <strong>${esc(formatDate(scheduleDate))}</strong>. Prepare items for loading at ${esc(p.branchName ?? 'your branch')}.`
        : `Order <strong>${esc(p.orderNumber)}</strong> for your customer <strong>${esc(p.customerName ?? 'the customer')}</strong> has been scheduled for delivery on <strong>${esc(formatDate(scheduleDate))}</strong>.`;
  ...
  const notesBlock = p.orderNotes?.trim()
    ? infoCard('Order notes', detailRow('Notes', p.orderNotes.trim()), `${p.orderNumber} · scheduled · notes`)
    : '';

  const ctaLabel = target === 'warehouse' ? 'View order' : 'Open order';
  const url = orderUrl(p.orderId);
  const refLabel =
    target === 'executive' ? 'Scheduled' : target === 'warehouse' ? 'Prepare for loading' : 'Scheduled for customer';
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.08);border:1px solid #e5e7eb;">
        ...
        <tr>
          <td style="padding:28px 32px;">
            ...
            ${infoCard('Order summary', orderSummaryRows, `${p.orderNumber} · scheduled · summary`)}
            ${logisticsRows ? infoCard('Delivery & payment', logisticsRows, `${p.orderNumber} · scheduled · delivery`) : ''}
            ${notesBlock}
            ${sectionTitle('Next step', `${p.orderNumber} · scheduled`)}
            <div style="text-align:center;margin-top:8px;">
              <a href="${url}" style="display:inline-block;background:#111827;color:#ffffff;text-decoration:none;padding:13px 32px;border-radius:8px;font-weight:600;font-size:15px;box-shadow:0 1px 2px rgba(0,0,0,.06);">${ctaLabel}</a>
            </div>
          </td>
        </tr>
        <tr>
