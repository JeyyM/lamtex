...
import { toggleEmployeesPermission } from '@/src/lib/permissions/employeeEmployeesPermissions';

const PERMISSION_ICONS: Record<EmployeesPermissionKey, LucideIcon> = {
  pageAccess: Eye,
};

export function EmployeesPermissionToggleGrid({
  value,
  onChange,
  disabled,
}: {
  value: EmployeesPermissionSet;
  onChange: (next: EmployeesPermissionSet) => void;
  disabled?: boolean;
}) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
      {EMPLOYEES_PERMISSIONS.map((def) => {
        ...
        return (
          <button
            ...
            aria-pressed={on}
            onClick={() => onChange(toggleEmployeesPermission(value, def.key))}
            className={cn(
...
export function EmployeesPermissionSection({
  title = 'Employees',
  description = 'Control access to the Employees directory and employee detail pages.',
  value,
  savedValue,
  onChange,
  onSave,
  ...
  description?: string;
  value: EmployeesPermissionSet;
  savedValue: EmployeesPermissionSet;
  onChange: (next: EmployeesPermissionSet) => void;
    ...
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="p-4 sm:p-6">
        {loading ? (
          ...
          <>
            <EmployeesPermissionToggleGrid value={value} onChange={onChange} disabled={saving} />
