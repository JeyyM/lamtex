...
export interface ProductionRequestAcceptedEmailPayload extends ProductionRequestSubmittedEmailPayload {
...
/** Email to the user who submitted a PR when an executive accepts it. */
export function buildProductionRequestAcceptedEmailHtml(p: ProductionRequestAcceptedEmailPayload): string {
  ...
  const notesBlock = p.notes?.trim()
    ? infoCard('Notes', detailRow('Notes', p.notes.trim()), `${p.prNumber} · accepted · notes`)
    : '';
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.08);border:1px solid #e5e7eb;">
        ...
        <tr>
          <td style="padding:28px 32px;">
            ...
            ${infoCard('Production request summary', summaryRows, `${p.prNumber} · accepted · summary`)}
            ${notesBlock}
