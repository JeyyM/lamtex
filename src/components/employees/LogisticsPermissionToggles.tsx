...
import { toggleLogisticsPermission } from '@/src/lib/permissions/employeeLogisticsPermissions';

const PERMISSION_ICONS: Record<LogisticsPermissionKey, LucideIcon> = {
  pageAccess: Eye,
};

export function LogisticsPermissionToggleGrid({
  value,
  onChange,
  disabled,
}: {
  value: LogisticsPermissionSet;
  onChange: (next: LogisticsPermissionSet) => void;
  disabled?: boolean;
}) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
      {LOGISTICS_PERMISSIONS.map((def) => {
        ...
        return (
          <button
            ...
            aria-pressed={on}
            onClick={() => onChange(toggleLogisticsPermission(value, def.key))}
            className={cn(
...
export function LogisticsPermissionSection({
  title = 'Logistics',
  description = 'Control access to the Logistics page and truck detail views.',
  value,
  savedValue,
  onChange,
  onSave,
  ...
  description?: string;
  value: LogisticsPermissionSet;
  savedValue: LogisticsPermissionSet;
  onChange: (next: LogisticsPermissionSet) => void;
