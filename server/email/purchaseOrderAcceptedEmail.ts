...
export interface PurchaseOrderAcceptedEmailPayload extends PurchaseOrderSubmittedEmailPayload {
...
/** Email to the warehouse user who submitted a PO when it is accepted. */
export function buildPurchaseOrderAcceptedEmailHtml(p: PurchaseOrderAcceptedEmailPayload): string {
  ...
  const notesBlock = p.notes?.trim()
    ? infoCard('Notes', detailRow('Notes', p.notes.trim()), `${p.poNumber} · accepted · notes`)
    : '';
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.08);border:1px solid #e5e7eb;">
        <tr>
          <td style="background:linear-gradient(135deg,#059669,#047857);padding:28px 32px;">
            ...
            <p style="margin:8px 0 0;color:#d1fae5;font-size:14px;">${esc(p.poNumber)} · ${esc(p.branchName ?? 'Branch')}</p>
          </td>
        </tr>
        <tr>
          <td style="padding:28px 32px;">
            ...
            ${infoCard('Purchase order summary', summaryRows, `${p.poNumber} · accepted · summary`)}
            ${notesBlock}
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e5e7eb;border-radius:10px;overflow:hidden;font-size:14px;margin-bottom:8px;">
              ...
              <tfoot>
                <tr style="background:#ecfdf5;">
                  ...
                  <td style="padding:14px;font-weight:700;color:#047857;text-align:right;font-size:17px;">${formatMoney(displayTotal, p.currency)}</td>
                </tr>
              </tfoot>
            </table>
