-- Seed fleet: trucks only (vehicle_type = Truck), per Lamtex branch codes MNL / CEB / BTG.
-- Requires: branches seeded, `database/fleet_trucks_extension.sql` applied.
-- Safe to re-run: uses ON CONFLICT (vehicle_id) DO UPDATE for idempotent refreshes.
...
FROM (VALUES
  -- Manila (NCR)
  ('TRK-MNL-001', 'Manila Rigid 001', 'NCR-1001', 'Truck'::vehicle_type, 'On Trip'::vehicle_status, 'Owned'::financing_status, 5000, 25, 78, 1, '2026-06-15'::date, ARRAY['High weekly utilization']::text[], 'Isuzu', 'Forward F-Series', 2022, 'White', 'OR-MNL-1001', '2027-03-01'::date, 45230),
  ('TRK-MNL-002', 'Manila Rigid 002', 'NCR-1002', 'Truck'::vehicle_type, 'Available'::vehicle_status, 'Financed'::financing_status, 5000, 25, 62, 0, '2026-08-01'::date, NULL, 'Mitsubishi', 'Fuso Fighter', 2021, 'Blue', 'OR-MNL-1002', '2026-09-10'::date, 62450),
  ('TRK-MNL-003', 'Manila Rigid 003', 'NCR-1003', 'Truck'::vehicle_type, 'Loading'::vehicle_status, 'Owned'::financing_status, 5000, 25, 71, 0, NULL, NULL, 'Hino', '500 Series', 2023, 'White', 'OR-MNL-1003', '2028-01-20'::date, 28940),
  ('TRK-MNL-004', 'Manila Wing Van 004', 'NCR-1004', 'Truck'::vehicle_type, 'Maintenance'::vehicle_status, 'Owned'::financing_status, 4000, 22, 45, 0, '2026-04-20'::date, ARRAY['Scheduled service']::text[], 'Isuzu', 'Giga', 2020, 'Silver', 'OR-MNL-1004', '2026-05-01'::date, 91200),
  -- Cebu
  ('TRK-CEB-001', 'Cebu Rigid 001', 'CEB-2001', 'Truck'::vehicle_type, 'On Trip'::vehicle_status, 'Owned'::financing_status, 5000, 25, 74, 1, '2026-07-01'::date, NULL, 'Fuso', 'Canter', 2022, 'White', 'OR-CEB-2001', '2027-08-15'::date, 33100),
  ('TRK-CEB-002', 'Cebu Rigid 002', 'CEB-2002', 'Truck'::vehicle_type, 'Available'::vehicle_status, 'Leased'::financing_status, 5000, 25, 58, 0, NULL, NULL, 'Isuzu', 'NPR', 2021, 'White', 'OR-CEB-2002', '2026-11-01'::date, 40200),
  ('TRK-CEB-003', 'Cebu Wing Van 003', 'CEB-2003', 'Truck'::vehicle_type, 'Available'::vehicle_status, 'Owned'::financing_status, 3500, 18, 55, 0, '2026-09-15'::date, NULL, 'Hino', '300 Series', 2019, 'Blue', 'OR-CEB-2003', '2026-10-01'::date, 77800),
  -- Batangas / Calabarzon
  ('TRK-BTG-001', 'Batangas Rigid 001', 'BTG-3001', 'Truck'::vehicle_type, 'Available'::vehicle_status, 'Owned'::financing_status, 8000, 32, 68, 0, '2026-05-30'::date, NULL, 'Isuzu', 'Giga', 2023, 'White', 'OR-BTG-3001', '2028-02-01'::date, 15400),
  ('TRK-BTG-002', 'Batangas Trailer 002', 'BTG-3002', 'Truck'::vehicle_type, 'Available'::vehicle_status, 'Financed'::financing_status, 12000, 45, 52, 0, NULL, NULL, 'Volvo', 'FM', 2022, 'Red', 'OR-BTG-3002', '2027-06-30'::date, 22100),
  ('TRK-BTG-003', 'Batangas Rigid 003', 'BTG-3003', 'Truck'::vehicle_type, 'Loading'::vehicle_status, 'Owned'::financing_status, 5000, 25, 66, 0, '2026-08-20'::date, NULL, 'Hino', '700 Series', 2021, 'White', 'OR-BTG-3003', '2026-12-12'::date, 49800)
) AS x(
...
-- ---------------------------------------------------------------------------
-- Active trips (link first "On Trip" truck per region)
-- ---------------------------------------------------------------------------
INSERT INTO trips (
  trip_number,
  vehicle_id,
...
-- Loading trucks: tie to planned trips
INSERT INTO trips (
  trip_number,
  vehicle_id,
  vehicle_name,
...
-- ---------------------------------------------------------------------------
-- Trip history (snapshot rows; trip_id left null)
-- ---------------------------------------------------------------------------
DELETE FROM trip_history WHERE trip_number LIKE 'TRP-%-HIST-%';
...
-- Maintenance records
-- ---------------------------------------------------------------------------
DELETE FROM maintenance_records WHERE description LIKE 'Fleet seed:%';
