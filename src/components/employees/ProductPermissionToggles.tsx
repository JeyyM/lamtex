...
import { toggleProductPermission } from '@/src/lib/permissions/employeeProductPermissions';

const PERMISSION_ICONS: Record<ProductPermissionKey, LucideIcon> = {
  pageAccess: Eye,
  stockAccess: Box,
  paymentData: DollarSign,
  categoryCreation: FolderPlus,
  exportAccess: Download,
  productCreation: PlusCircle,
  priceModification: Tag,
  dataAndStatistics: BarChart3,
  activityLog: Clock,
  productionRequestsHistory: ClipboardList,
};

export interface ProductPermissionToggleGridProps {
  value: ProductPermissionSet;
  onChange: (next: ProductPermissionSet) => void;
  disabled?: boolean;
}

export function ProductPermissionToggleGrid({ value, onChange, disabled }: ProductPermissionToggleGridProps) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
      {PRODUCT_PERMISSIONS.map((def) => {
        ...
        return (
          <button
            ...
            aria-pressed={on}
            onClick={() => onChange(toggleProductPermission(value, def.key))}
            className={cn(
...
export interface ProductPermissionSectionProps {
  title?: string;
  description?: string;
  value: ProductPermissionSet;
  savedValue: ProductPermissionSet;
  onChange: (next: ProductPermissionSet) => void;
...
export function ProductPermissionSection({
  title = 'Products',
  description = 'Control what this employee can see and do on the Products catalog, category, and family pages.',
  value,
  savedValue,
  onChange,
  onSave,
    ...
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="p-4 sm:p-6">
        {loading ? (
          ...
          <>
            <ProductPermissionToggleGrid value={value} onChange={onChange} disabled={saving} />
