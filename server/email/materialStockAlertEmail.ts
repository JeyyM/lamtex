...
export interface MaterialStockAlertEmailPayload {
  materialId: string;
  name: string;
  sku: string;
  unit: string | null;
...
function severityLabel(s: MaterialStockAlertSeverity): string {
  switch (s) {
    case 'out_of_stock':
      return 'Out of stock';
    case 'critical':
...
function severityHeaderGradient(s: MaterialStockAlertSeverity): string {
...
function severityHeaderSubtle(s: MaterialStockAlertSeverity): string {
  if (s === 'out_of_stock' || s === 'critical') return '#fecaca';
  return '#fde68a';
}

function materialPageUrl(materialId: string): string {
  const appUrl = process.env.APP_URL ?? 'http://localhost:3000';
  return `${appUrl.replace(/\/$/, '')}/materials/${materialId}`;
}

function ctaLabel(audience: MaterialStockAlertAudience): string {
...
/** Inventory alert email — fires when a raw material crosses a stock threshold. */
export function buildMaterialStockAlertEmailHtml(p: MaterialStockAlertEmailPayload): string {
  ...
  const bodyMessage =
    p.severity === 'out_of_stock'
      ? `${esc(p.sku)} · ${esc(p.name)} is <strong>out of stock</strong>${esc(branchSuffix)}.`
      : p.severity === 'critical'
        ? `${esc(p.sku)} · ${esc(p.name)} is at <strong>critical stock</strong>${esc(branchSuffix)}: ${formatQty(p.newStock, p.unit)} on hand (reorder at ${formatQty(p.reorderPoint, p.unit)}).`
        : `${esc(p.sku)} · ${esc(p.name)} is <strong>below reorder point</strong>${esc(branchSuffix)}: ${formatQty(p.newStock, p.unit)} on hand (reorder at ${formatQty(p.reorderPoint, p.unit)}).`;
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.08);border:1px solid #e5e7eb;">
        <tr>
          <td style="background:${headerGradient};padding:28px 32px;">
            ...
            <p style="margin:8px 0 0;color:${headerSubtle};font-size:14px;">${esc(p.sku)} · ${esc(p.name)}${esc(branchSuffix)}</p>
          </td>
        </tr>
        <tr>
          <td style="padding:28px 32px;">
            ${emailRefLine(p.sku, `Material alert · ${severityText}`)}
            <p style="margin:0 0 20px;color:#374151;font-size:15px;line-height:1.55;">
              ${bodyMessage}
            </p>
            ${infoCard('Stock snapshot', summaryRows, `${p.sku} · ${p.materialId}`)}
            ...
            <div style="text-align:center;">
              <a href="${materialPageUrl(p.materialId)}" style="display:inline-block;background:#111827;color:#ffffff;text-decoration:none;padding:13px 32px;border-radius:8px;font-weight:600;font-size:15px;box-shadow:0 1px 2px rgba(0,0,0,.06);">${esc(ctaLabel(p.audience))}</a>
            </div>
          </td>
        </tr>
        <tr>
          <td style="padding:16px 32px;background:#f9fafb;border-top:1px solid #e5e7eb;color:#9ca3af;font-size:12px;text-align:center;">
            Lamtex · Raw material notification
          </td>
        </tr>
      </table>
    </td></tr>
