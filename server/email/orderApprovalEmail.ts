...
export function buildOrderApprovalEmailHtml(p: OrderCreatedEmailPayload): string {
  ...
  const notesBlock = p.orderNotes?.trim()
    ? infoCard('Order notes', detailRow('Notes', p.orderNotes.trim()), `${p.orderNumber} · pending approval · notes`)
    : '';
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.08);border:1px solid #e5e7eb;">
        ...
        <tr>
          <td style="padding:28px 32px;">
            ...
            ${infoCard('Order summary', orderSummaryRows, `${p.orderNumber} · pending approval · summary`)}
            ${infoCard('Customer & agent', peopleRows, `${p.orderNumber} · pending approval · customer & agent`)}
            ${logisticsRows ? infoCard('Delivery & payment', logisticsRows, `${p.orderNumber} · pending approval · delivery`) : ''}
            ${notesBlock}
            ...
            <div style="margin-top:28px;text-align:center;">
              <a href="${orderUrl}" style="display:inline-block;background:#dc2626;color:#ffffff;text-decoration:none;padding:13px 32px;border-radius:8px;font-weight:600;font-size:15px;box-shadow:0 1px 2px rgba(0,0,0,.06);">Review &amp; approve order</a>
            </div>
          </td>
        </tr>
        <tr>
