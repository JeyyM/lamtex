...
import { Bell, Building2, CreditCard, Globe, Loader2, MapPin } from 'lucide-react';
...
import { toggleSettingsPermission } from '@/src/lib/permissions/employeeSettingsPermissions';

const PERMISSION_ICONS: Record<SettingsPermissionKey, LucideIcon> = {
  companyInfo: Building2,
  addresses: MapPin,
  paymentProfiles: CreditCard,
  socialMedia: Globe,
  notifications: Bell,
};

export function SettingsPermissionToggleGrid({
  value,
  onChange,
  disabled,
}: {
  value: SettingsPermissionSet;
  onChange: (next: SettingsPermissionSet) => void;
  disabled?: boolean;
}) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
      {SETTINGS_PERMISSIONS.map((def) => {
        ...
        return (
          <button
            ...
            aria-pressed={on}
            onClick={() => onChange(toggleSettingsPermission(value, def.key))}
            className={cn(
          ...
          >
            ...
            <span className={cn('text-[10px] font-medium uppercase tracking-wide', on ? 'text-red-600' : 'text-gray-400')}>
              {on ? 'On' : 'Off'}
            </span>
          </button>
        );
...
export function SettingsPermissionSection({
  title = 'Settings',
  description = 'Control access to each Settings tab (company info, addresses, payment profiles, social media, notifications).',
  value,
  savedValue,
  onChange,
  onSave,
  ...
  description?: string;
  value: SettingsPermissionSet;
  savedValue: SettingsPermissionSet;
  onChange: (next: SettingsPermissionSet) => void;
    ...
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="p-4 sm:p-6">
        {loading ? (
          ...
          <>
            <SettingsPermissionToggleGrid value={value} onChange={onChange} disabled={saving} />
