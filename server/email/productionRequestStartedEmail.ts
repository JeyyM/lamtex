...
export interface ProductionRequestStartedEmailPayload extends ProductionRequestSubmittedEmailPayload {
...
/** Email to warehouse staff and executives when PR production starts. */
export function buildProductionRequestStartedEmailHtml(p: ProductionRequestStartedEmailPayload): string {
  ...
  const notesBlock = p.notes?.trim()
    ? infoCard('Notes', detailRow('Notes', p.notes.trim()), `${p.prNumber} · in progress · notes`)
    : '';
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.08);border:1px solid #e5e7eb;">
        <tr>
          <td style="background:linear-gradient(135deg,#2563eb,#1d4ed8);padding:28px 32px;">
            ...
            <p style="margin:8px 0 0;color:#dbeafe;font-size:14px;">${esc(p.prNumber)} · ${esc(p.branchName ?? 'Branch')}</p>
          </td>
        </tr>
        <tr>
          <td style="padding:28px 32px;">
            ...
            ${infoCard('Production request summary', summaryRows, `${p.prNumber} · in progress · summary`)}
            ${notesBlock}
